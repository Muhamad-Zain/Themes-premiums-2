...
const fetchBg = async (id) => {
    try {
        const galleryRef = storageRef(storage, `weddings-two/${id}`); // Referensi ke folder '01/galery'
...
const fetchGalery = async (id) => {
    try {
        const galleryRef = storageRef(storage, `weddings-two/${id}/galery`); // Referensi ke folder '01/galery'
